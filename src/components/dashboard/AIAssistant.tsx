  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'ai',
      content: "Hello! I'm your AI DevOps assistant. I can help you deploy applications, manage infrastructure, scale resources, and optimize costs. What would you like to do today?",
      timestamp: new Date(),
    }
  ]);
              ...
              <div className={`inline-block p-4 rounded-2xl ${
                ...
                {message.status && (
                  <div className="flex items-center space-x-1 mt-2 text-sm">
                    ...
                    <span className="text-gray-400 capitalize">{message.status}</span>
                  </div>
                )}
              </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {quickActions.map((action, index) => (
            <button
              ...
              className="p-3 bg-white/5 hover:bg-white/10 rounded-lg text-left text-sm text-gray-300 hover:text-white transition-all border border-white/10 hover:border-purple-400/50"
            >
              <Zap className="h-4 w-4 inline mr-2 text-purple-400" />
              {action}
            </button>
          ))}
        </div>
        ...
        <div className="flex space-x-2">
          <input
            ...
            className="flex-1 p-4 bg-white/10 rounded-xl text-white placeholder-gray-400 border border-white/20 focus:border-purple-400 focus:outline-none"
          />
          <button
            onClick={handleSendMessage}
            disabled={!inputValue.trim()}
            className="p-4 bg-gradient-to-r from-purple-500 to-blue-500 rounded-xl hover:from-purple-600 hover:to-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
